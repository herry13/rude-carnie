...
def classify_many_single_crop(sess, label_list, softmax_output, coder, images, image_files, writer):
    try:
        ...
        for j in range(num_batches):
            ...
            tf.logging.info(start_offset, end_offset, len(batch_image_files))
            ...
            for i in range(batch_sz):
                ...
                tf.logging.info('Guess @ 1 %s, prob = %.2f' % best_choice)
...
def classify_one_multi_crop(sess, label_list, softmax_output, coder, images, image_file, writer):
    try:
        ...
        if nlabels > 2:
            ...
            tf.logging.info('Guess @ 2 %s, prob = %.2f' % (label_list[second_best], output[second_best]))

        if writer is not None:
            writer.writerow((f, best_choice[0], '%.2f' % best_choice[1]))
    except Exception as e:
        tf.logging.error(e)
        tf.logging.error('Failed to run image %s ' % image_file)
...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    if FLAGS.face_detection_model:
        tf.logging.info('Using face detector (%s) %s' % (FLAGS.face_detection_type, FLAGS.face_detection_model))
    ...
    with tf.Session(config=config) as sess:
        ...
        with tf.device(FLAGS.device_id):
            ...
            # load the model from checkpoint files
            ...
            # print timer
            tf.logging.info('TensorFlow model was loaded in %.6f seconds' % (timeit.default_timer() - timer))
            ...
            output = None
            if FLAGS.target:
                tf.logging.info('Creating output file %s' % FLAGS.target)
            ...
            tf.logging.info('Classification was done in %.6f seconds' % (timeit.default_timer() - timer))
